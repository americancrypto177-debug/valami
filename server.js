...
// Active users/presence (operators + portal users) – cross-device moderation UI
app.get('/api/active-users', async (_req, res) => {
  try{
    const t = now();

    // Precompute operator load (number of assigned threads)
    const store = await loadThreads().catch(()=>({ threads: {}, assignments: {}, queue: [] }));
    const loadByOp = {};
    for(const tid of Object.keys(store.assignments||{})){
      const op = lower(store.assignments[tid]);
      if(!op) continue;
      loadByOp[op] = (loadByOp[op]||0) + 1;
    }

    // Operators
    const ops = [];
    const opKeys = new Set([
      ...Array.from(presence.operators.keys()),
      ...Array.from(socketsByOperator.keys())
    ]);
    for(const email of opKeys){
      const e = lower(email);
      if(!e) continue;
      const lastSeen = presence.operators.get(e)?.lastSeen || 0;
      const online = isOnlineOperator(e);
      const load = loadByOp[e] || 0;
      ops.push({ email: e, operatorId: e, lastSeen, online, load, available: online && load === 0 });
    }

    // Portal users (currently connected)
    const users = [];
    for(const u of presence.users.values()){
      const uid = lower(u?.userId);
      if(!uid) continue;
      const lastSeen = u?.lastSeen || 0;
      const online = (t - lastSeen) < ONLINE_TTL_MS;
      users.push({ userId: uid, lastSeen, online, nick: u?.nick || '', city: u?.city || '', profilePic: u?.profilePic || '' });
    }

    res.json({ ok:true, operators: ops.sort((a,b)=>a.email.localeCompare(b.email)), users });
  }catch(e){
    res.status(500).json({ ok:false, error:'active_users_failed' });
  }
});
...
function isOnlineOperator(operatorId){
  // Treat an operator as online if:
  // - we have an active socket for them, OR
  // - their presence ping is within TTL.
...
async function pickOperator(){
  // pick an online operator that currently has ZERO active assignments
  const store = await loadThreads();
  const assignments = store.assignments || {};
  const load = {};
  for(const tid of Object.keys(assignments)){
    const op = lower(assignments[tid]);
    if(!op) continue;
    load[op] = (load[op]||0) + 1;
  }

  // Some UIs may connect but not send a presence ping immediately.
  // Merge both sources so routing still works.
  const online = Array.from(new Set([
    ...Array.from(presence.operators.keys()).map(lower),
    ...Array.from(socketsByOperator.keys()).map(lower)
  ])).filter(isOnlineOperator);
...
socket.data.age = p?.age ?? '';
  ...
  socket.on('operator:handled', async (p) => {
    // operator finished current; release thread assignment so the operator becomes eligible again
    if(socket.data.role != 'operator') return;
    const op = lower(socket.data.email);
    touchOperator(op);
    ...
    socket.emit('operator:backToWaiting', { ok:true, threadId: threadId || '' });
    await flushQueue();
  });
...
const adminEmail = 'admin@forrovagy.hu';
const msgEmail = lower(p?.email || p?.userMeta?.email || '');
const isAdmin = (lower(socket.data?.email) === adminEmail) || (msgEmail === adminEmail) || (lower(userId) === adminEmail);
const cost = isAdmin ? 0 : 10;
